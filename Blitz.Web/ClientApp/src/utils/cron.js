...
/**
 * @param {string} exp
 * @return {{minutes, hours, days, months, weekDays} | undefined}
 * */
...
export function nextDate(exp, from = new Date()) {
 ...
 * @return {Date[]}
 */
export function calculateNextDates(exp, count = 3, from = new Date()) {
    const cron = parseCron(exp);
    if (!cron) return [];
